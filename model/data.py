...
class CatPhotoDataset(Dataset):
    def __init__(self, data: List[Tuple[str, int, float]], size: int, num_classes: int, balance_num: int,
        ...
        if augmentation:
            self.transform = transforms.Compose([
                ...
                transforms.RandomCrop(size, padding=size // 8, fill=0),
                # 四周填充1/8大小后再随机裁剪为(size,size)
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        while True:
            try:
                ...
                img = Image.open(img_path).convert('RGB').resize((self.size, self.size)) # 使用 self.size
                break
            except:
                continue
...
class CatPhotoDataModule(LightningDataModule):
    def __init__(self, root: str, size: int, filter_num: int, balance_num: int, batch_size: int):
        ...
        self.train_dataset = CatPhotoDataset(train_data, self.size, len(self.cat_ids), balance_num,
                                              augmentation=True)  # 传递 self.size
        self.val_dataset = CatPhotoDataset(val_data, self.size, len(self.cat_ids), math.ceil(balance_num * 0.2))  # 传递 self.size
